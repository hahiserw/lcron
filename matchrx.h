...
/*
int
find_rx(const char *rx, char *string, int start, *char output)
{
	int r, n;
	regex_t crx;
	regmatch_t sub_offset;
	memset(&sub_offer, 0, sizeof(regmatch_t));
	sub_offset.rm_so = start;

	if (regcomp(&crx, rx, REG_EXTENDED))
		return -1;
	r = regexec(&crx, string, 2, sub_offset, 0);
